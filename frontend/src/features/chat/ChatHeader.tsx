...
export function ChatHeader({
  ...
  return (
    <header className="flex h-16 shrink-0 items-center justify-between gap-2 border-b border-border/60 bg-background/80 px-2 backdrop-blur sm:gap-4 sm:px-4">
      ...
      <div className="flex shrink-0 items-center gap-2">
        {totals && can(user, 'see_usage') && <ConversationUsage totals={totals} />}
        {student && <TextSizeControl compact />}
        {conversationId && can(user, 'share_conversations') && (
          <Button variant="ghost" size="sm" onClick={onShare} title="Share a public link">
        ...
        {conversationId && !student && (
          ...
          <a href={`/api/conversations/${conversationId}/export`} download title="Export as Markdown">
